...
\textcolor{red}{CAN THIS BE CALLED LEMMA?}\\
...
for(auto it = g.vertices.lbegin(); it != g.vertices.lend(); ++it) {
  Graph::vertex_type v = *it;
  for(auto it2 = g.out_edges.vbegin(v); it2 != g.out_edges.vend(v); ++it2) {
...
\autoref{attributed_graph_code} shows a graph being constructed with 2 vertices and en edge between them. The Vertices and edges all hold an attribute \textit{id}. Because the syntax is rather complicated it is advised to implement an \texttt{AttributedDynamicGraph} instead, if feasible.
...
\subsection{AttributedDynamicGraph}
\label{attributed_dynamic_graph_section}
The \texttt{AttributedDynamicGraph} concept is a specialization of the concepts \texttt{AttributedGraph} and \texttt{DynamicGraph}. It extends the concepts with functionality for adding attributed graph elements dynamically. Methods for adding vertices and edges are extended with references to user-defined attribute structs.

\autoref{dynamic_attributed_graph_code} shows an empty graph being constructed and two vertices as well as one edge between them added. Vertices and edges contain an attribute \textit{id}.
...
v_prop vp1 { 1 };
auto v1 = g.add_vertex(vp1);
v_prop vp2 { 2 };
auto v2 = g.add_vertex(vp2);
e_prop ep1 { 1 };
auto e1 = g.add_edge(v1, v2, ep1);
\end{lstlisting}
...
Graph::vertex_type v = *it; // get first vertex (vertex 1)

for(auto it2 = g.in_edges.vbegin(v); it2 != g.in_edges.vend(v); ++it2) {
...
\begin{lstlisting}[caption=Combined edge iteration\label{duplex_code}] 
std::list<std::pair<int, int>> edge_list = { {1, 2}, {1, 3}, {4, 1} };
Graph g(4, 3, edge_list.begin(), edge_list.end());
...
for(auto it2 = g.edges.vbegin(v); it2 != g.edges.vend(v); ++it2) {
...
\item $it_1$ and $it_2$, \texttt{InputIterators}\footnote[2]{see \autoref{stl_iterator_section}} with a valid range $[i_1, i_2]$ referring to elements of type \texttt{std::pair<int, int>}
\item $v$, a value of type \texttt{X::vertex\_type}
\item $i_v$, a value of type \texttt{X::vertex\_index\_type}
\end{itemize}
...
\begin{tabularx}{1\textwidth}{| X | X | X |}
  ...
  \endhead  
  \hline
  $vertex\_type$ & a type holding vertex information &  \\ \hline
  $edge\_type$ & a type holding edge information &  \\ \hline
  $vertex\_size\_type$ & unsigned integer &  \\ \hline
  $edge\_size\_type$ & unsigned integer &  \\ \hline
  $vertex\_index\_type$ & integer &  \\ \hline
  $edge\_index\_type$ & integer &  \\ \hline
...
\begin{tabularx}{1\textwidth}{| l | l | X |}
  ...
  \endhead
  \hline
  $X(n_v, n_e, it_1, it_2)$ &  & constructs the graph holding $n_v$ vertices and $n_e$ edges given by the iterators $i_1$ and $i_2$ of an edge list container\\ \hline
  ...
  $x.out_edges.vbegin(v)$ & global\_out\_edge\_iterator & iterator to the first outbound edge connected to vertex $v$ in global iteration space \\ \hline
  $x.out\_edges.vend(v)$ & global\_out\_edge\_iterator & iterator past the last outbound edge connected to vertex $v$ in global iteration space \\ \hline
  $x.out\_edges.vbegin(i_v)$ & global\_out\_edge\_iterator & iterator to the first outbound edge connected to the vertex identified by $i_v$ in global iteration space \\ \hline
  $x.out\_edges.vend(i_v)$ & global\_out\_edge\_iterator & iterator past the last outbound edge connected to to the vertex identified by $i_v$ in global iteration space \\ \hline
\end{tabularx}
...
\begin{tabularx}{1\textwidth}{| l | X | X |}
  ...
  \endhead
  \hline
  $X(n_v, n_e, it_1, it_2)$ & $n_e$ == std::distance($it_1$, $it_2$),\newline$n_v$ == the amount of different integer values in elements of $[it_1, it_2]$ & std::distance(x.vertices.\newline begin(), x.vertices.end()) == $n_v$\newline std::distance(x.out\_edges.\newline  begin(), x.out\_edges.end()) == $n_e$\\ \hline
  ...
  $x.out_edges.vbegin(v)$ & $v$ is a valid vertex in X & \\ \hline
  $x.out\_edges.vend(v)$ & $v$ is a valid vertex in X & \\ \hline
  $x.out\_edges.vbegin(i_v)$ & $i_v$ identifies a valid vertex in X & \\ \hline
  $x.out\_edges.vend(i_v)$ & $i_v$ identifies a valid vertex in X & \\ \hline
\end{tabularx}
...
\item $v$, $v_1$ and $v_2$, values of type \texttt{X::vertex\_type}
\item $e$, a value of type \texttt{X::edge\_type}
\item $i_v$, $i_{v1}$ and $i_{v2}$, values of type \texttt{X::vertex\_index\_type}
\item $i_e$, a value of type \texttt{X::edge\_index\_type}
\end{itemize}
...
\begin{tabularx}{1\textwidth}{| l | l | X |}
  ...
  $x.add\_vertex()$ & vertex\_index\_type & adds a vertex \\ \hline
  $x.remove\_vertex(v)$ &  & removes vertex $v$ \\ \hline
  $x.remove\_vertex(i_v)$ &  & removes a vertex identified by $i_v$ \\ \hline
  $x.clear\_vertex(v)$ &  & removes all outgoing edges from vertex $v$ \\ \hline
  $x.clear\_vertex(i_v)$ &  & removes all outgoing edges from a vertex identified by $i_v$ \\ \hline
  $x.add\_edge(v_1, v_2)$ & edge\_index\_type & adds an edge between vertices $v_1$ and $v_2$ \\ \hline
  $x.add\_edge(i_{v1}, i_{v2})$ & edge\_index\_type & adds an edge between vertices identified by $i_{v1}$ and $i_{v2}$ \\ \hline
  $x.remove\_edge(e)$ &  & removes edge $e$ \\ \hline
  $x.remove\_edge(i_e)$ &  & removes an edge identified by $i_e$ \\ \hline
  $x.barrier()$ &  & synchronizes memory space across all units \\ \hline
\end{tabularx}
...
\begin{tabularx}{1\textwidth}{| l | X | X |}
  ...
  $x.add\_vertex()$ &  & returned index identifies constructed vertex in global index space \\ \hline
  $x.remove\_vertex(v)$ & $v$ is a valid vertex in X & \\ \hline
  $x.remove\_vertex(i_v)$ & $i_v$ identifies a valid vertex in X & \\ \hline
  $x.clear\_vertex(v)$ & $v$ is a valid vertex in X & \\ \hline
  $x.clear\_vertex(i_v)$ & $i_v$ identifies a valid vertex in X & \\ \hline
  $x.add\_edge(v_1, v_2)$ & $v_1$ and $v_2$ are valid vertices in X & returned index identifies constructed edge in global index space \\ \hline
  $x.add\_edge(i_{v1}, i_{v2})$ & $i_{v1}$ and $i_{v2}$ identify valid vertices in X & returned index identifies constructed edge in global index space \\ \hline
  $x.remove\_edge(e)$ & $e$ is a valid edge in X & \\ \hline
  $x.remove\_edge(i_e)$ & $i_e$ identifies a valid edge in X & \\ \hline
\end{tabularx}
...
\item $it_1$ and $it_2$, \texttt{InputIterators} with a valid range $[i_1, i_2]$ referring to elements of type \texttt{std::tuple<std::pair<int, VT>, std::pair<int, VT>, ET>}
\end{itemize}
...
\begin{tabularx}{1\textwidth}{| l | X | X |}
  ...
  \endhead  
  \hline
  $x.add\_vertex(a_v)$ & $a_v$ is \texttt{CopyInsertable} in X & returned index identifies constructed vertex in global index space \\ \hline
  $x.add\_edge(v_1, v_2, a_e)$ & $v_1$ and $v_2$ are valid vertices in X \newline $a_e$ is \texttt{CopyInsertable} in X & returned index identifies constructed edge in global index space \\ \hline
  $x.add\_edge(i_{v1}, i_{v2}, a_e)$ & $i_{v1}$ and $i_{v2}$ identify valid vertices in X \newline $a_e$ is \texttt{CopyInsertable} in X & returned index identifies constructed edge in global index space \\ \hline
\end{tabularx}

\section{AttributedDynamicGraph}
An \texttt{AttributedDynamicGraph} is a \texttt{DynamicGraph} containing arbitrary attributes for vertices and edges. An \texttt{AttributedDynamicGraph} also satisfies the \texttt{AttributedGraph} concept.
...
\item $v_1$ and $v_2$, values of type \texttt{X::vertex\_type}
\item $i_{v1}$ and $i_{v2}$, values of type \texttt{X::vertex\_index\_type}
\end{itemize}
...
\begin{tabularx}{1\textwidth}{| l | l | X |}
  ...
  \endhead  
  \hline
  $x.add\_vertex(a_v)$ & vertex\_index\_type & adds a vertex holding a copy of $a_v$ \\ \hline
  $x.add\_edge(v_1, v_2, a_e)$ & edge\_index\_type & adds an edge between vertices $v_1$ and $v_2$ holding a copy of $a_e$ \\ \hline
  $x.add\_edge(i_{v1}, i_{v2}, a_e)$ &  edge\_index\_type & adds an edge between vertices identified by $i_{v1}$ and $i_{v2}$ holding a copy of $a_e$ \\ \hline
\end{tabularx}
...
\begin{tabularx}{1\textwidth}{| l | l | X |}
  ...
  $x.in_edges.vbegin(v)$ & global\_in\_edge\_iterator & iterator to the first inbound edge connected to vertex $v$ in global iteration space \\ \hline
  $x.in\_edges.vend(v)$ & global\_in\_edge\_iterator & iterator past the last inbound edge connected to vertex $v$ in global iteration space \\ \hline
  $x.in\_edges.vbegin(i_v)$ & global\_in\_edge\_iterator & iterator to the first inbound edge connected to the vertex identified by $i_v$ in global iteration space \\ \hline
  $x.in\_edges.vend(i_v)$ & global\_in\_edge\_iterator & iterator past the last inbound edge connected to to the vertex identified by $i_v$ in global iteration space \\ \hline
\end{tabularx}
...
\begin{tabularx}{1\textwidth}{| l | X | X |}
  ...
  $x.in_edges.vbegin(v)$ & $v$ is a valid vertex in X & \\ \hline
  $x.in\_edges.vend(v)$ & $v$ is a valid vertex in X & \\ \hline
  $x.in\_edges.vbegin(i_v)$ & $i_v$ identifies a valid vertex in X & \\ \hline
  $x.in\_edges.vend(i_v)$ & $i_v$ identifies a valid vertex in X & \\ \hline
\end{tabularx}
...
\begin{tabularx}{1\textwidth}{| l | l | X |}
  ...
  $x.edges.vbegin(v)$ & global\_edge\_iterator & iterator to the first edge (inbound and outbound) connected to vertex $v$ in global iteration space \\ \hline
  $x.edges.vend(v)$ & global\_edge\_iterator & iterator past the last edge (inbound and outbound) connected to vertex $v$ in global iteration space \\ \hline
  $x.edges.vbegin(i_v)$ & global\_edge\_iterator & iterator to the first edge (inbound and outbound) connected to the vertex identified by $i_v$ in global iteration space \\ \hline
  $x.edges.vend(i_v)$ & global\_edge\_iterator & iterator past the last edge (inbound and outbound) connected to to the vertex identified by $i_v$ in global iteration space \\ \hline
\end{tabularx}
...
\begin{tabularx}{1\textwidth}{| l | X | X |}
  ...
  $x.edges.lend()$ & & \\ \hline
  $x.edges.vbegin(v)$ & $v$ is a valid vertex in X & \\ \hline
  $x.edges.vend(v)$ & $v$ is a valid vertex in X & \\ \hline
  $x.edges.vbegin(i_v)$ & $i_v$ identifies a valid vertex in X & \\ \hline
  $x.edges.vend(i_v)$ & $i_v$ identifies a valid vertex in X & \\ \hline
\end{tabularx}
