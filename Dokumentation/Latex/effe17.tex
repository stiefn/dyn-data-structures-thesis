...
\begin{tabularx}{1\textwidth}{| X | X |}
  ...
  Global, asynchronous attribute access & access to attributes belopnging to particular elements on other machines without synchronization enforcement \\ \hline
  ...
  Container meta-data retrieval & Retrieval of container metadata like \textbf{number of vertices} during runtime \\ \hline
\end{tabularx}
...
\textcolor{red}{INTEGRATE TEAM CONCEPT, EXPLAIN UNIT CONCEPT}
...
\autoref{adj_it_code} shows an adjacency iteration over all outgoing edges of local vertices. Since all edges belonging to a particular vertex reside on the same unit, only local iterators are available for adjacency iteration of a given vertex identified by a local iterator (\texttt{it}).
...
Dynamic data changes of the graph are not reflected in global memory space until the user explicitly issues a \texttt{commit} operation. Before that, the changes can be only seen on the same unit using local iterators. A \texttt{commit} operation can invalidate global iterators.
...
The concept of this work does not comprise index spaces. They can however be implemented on top of it and exist alongside the iteration spaces that are primarily used to identify elements. This section gives a brief overview of possibilities in this regard:\\
...
\textit{Variant 1} requires communication between units every time an element is created. This additional overhead is intolerable for high performance applications and therefore not suitable for this concept.\\

\textit{Variant 2} only adds communication during the \texttt{commit} operation. If new elements have been created, the \texttt{commit} operation executes all-to-all communication. An additional mechanism that negotiates index numbers could therefore be performed in the same communication steps adding only a slight overhead. Unfortunately, using the created elements locally before a \texttt{commit} requires the user to handle the elements with a local index that is later substituted by a global index. This leads to additional programming complexity.\\
...
\textit{Variant 3} allows for indices to be created without global consensus: Indices contain the unit ID and the local index on that unit. This however comes with another problem: If mechanisms for element migration are employed, the global (unit) part of the indices has to be either changed or a global directory structure has to be built in order to redirect requests to a certain index to the correct unit.
...
\begin{defi}
A vertex index is a split index tuple (unit ID, local index)
\end{defi}
...
Element deletion is at the time of this wirting not implemented. This section however describes how a deletion mechanism can be implemented.
...
The algorithm used in this case study is based on the shared-memory version found in \cite{pgas_algorithms}. Contrary to the classical variant of the algorithm, no depth-first search is performed. All edges of the graph are examined in an arbitrary order leading to much better locality exploitation: Each unit can just examine the edges in the order they are placed in the memory.

The algorithms requires vertex attributes for the parent component a vertex belongs to. This parent component attribute is initially set to the vertex itself. The algorithm then proceeds with iteration over the following two steps until no more component changes can be found:

\begin{enumerate}
\item For each edge, set the component of the source vertex' current parent to the target vertex' parent.
\item Perform pointer-jumping over all components until the parent components of all vertices point to themselves.
\end{enumerate} 

The described algorithm is then optimized using communication coalescing explained in \autoref{comm_coal_section}. The resulting algorithm can be seen in \autoref{cc_algorithm}. The components of the target vertices of all edges local to each unit are gathered in a bulk communication step. This data is used to set the parent component of each target's parent to the source's parent component in another bulk communication step. Pointer jumping is then performed in a synchronized way in order to allow for communication coalescing: The components of each vertex is updated and the pointers are not immediately followed but rather updated in the next iteration. The algorithm stops if there are no changes in the parent components of all vertices in an iteration. 
...
A \textit{minimum spanning tree} is a tree structure spanning a connected component along paths of minimum edge weight \cite{ita}. A graph containing multiple connected components contains a \textit{minimum spanning forest} of trees for each component. \autoref{mst_figure} shows a graph and its corresponding minimum spanning tree.
