...
Creation and deletion of elements happens locally on each processor. The resulting changes in local memory and iteration space are not reflected in the respective global spaces until the graph's \textit{barrier synchronization} is called. This \textit{epoch-based} synchronization allows for a trade-off between dynamics and performance.
...
Global and local iteration spaces directly map to global and local memory spaces explained in \cref{memspace_section}. Because of this, iterators are invalidated when changes to the memory spaces are published.
...
Index spaces are not specified in this work. This section however gives a brief overview how such index spaces can be created on top of the existing concepts:\\
...
\Cref{mst_boruvka_figure} a) depicts an exemplary graph that is distributed among two processors. \Cref{mst_boruvka_figure} b) shows the first iteration of the algorithm: For each vertex, the edges with minimum weight are chosen. The resulting connected components (1, 3, 5) and (2, 6, 4) are then compacted to the new supervertices 1' and 2' which are shown in \cref{mst_boruvka_figure} c). Because processor 1 and processor 2 see different edges between these supervertices, both processors choose a different edge as a candidate for the minimum weight edge between the them. To reach consensus, both processors exchange these edges along with their weight and the processor owning the edge with minimum weight updates the minimum spanning tree.\\
