...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.7\textwidth]{Bilder/cc_steps.pdf}
	\caption{Steps of connected components with a) initial setup, b) first step, c) second step}
	\label{cc_steps_figure}
\end{figure}

\autoref{cc_steps_figure} a) shows the initial setup for the connected components algorithm according to the example graph of \autoref{cc_figure}: Each vertex has a parent component attribute assigned to it that is initially set to the vertex itself. For each edge in the graph, the component of the target vertex component is set to the component of the source vertex if the target component is greater than the source component in \autoref{cc_steps_figure} b). Pointer-jumping (\texttt{parent[vertex] = parent[parent[vertex]]}) is then performed in \autoref{cc_steps_figure} c) so that each vertex either points to itself or to another vertex that points to itself. It is worth to note that, because edge-lists can be traversed in arbitrary order, the component of \textit{vertex 4} in the first algorithm step can also be \textit{2}. In this case, the pointer-jumping step will not perform any changes. After a second iteration, the component of \textit{vertex 2} will be set to \textit{1} resulting in termination of the algorithm.\\
...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.8\textwidth]{Bilder/mst_boruvka.pdf}
	\caption{Distributed minimum spanning tree example showing a) the graph, b) the first iteration, c) the second iteration \cite{boruvka}}
...
\autoref{mst_boruvka_figure} a) depicts an exemplary graph that is distributed among two units. \autoref{mst_boruvka_figure} b) shows the first iteration of the algorithm: For each vertex, the edges with minimum weight are chosen. The resulting connected components (1, 3, 5) and (2, 6, 4) are then compacted to the new supervertices 1' and 2' which are shown in \autoref{mst_boruvka_figure} c). Because unit 1 and unit 2 see different edges between these supervertices, both units choose a different edge as a candidate for the minimum weight edge between the supervertices. To reach consensus, both units exchange these edges along with their weight and the unit owning the edge with minimum weight updates the minimum spanning tree.\\
