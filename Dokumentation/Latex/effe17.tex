...
This section describes the graph container concepts that have been deduced from the related work analysis of \autoref{related_work_section} and the graph algorithm analysis of \autoref{graph_algo_section}. The concepts follow the C++ standard library's container concept schemes.
...
\subsection{Graph}
A \texttt{Graph} is a \texttt{Container}\footnote[1]{http://doc.dash-project.org/api/latest/html/group\_\_DashContainerConcept.html} providing minimal functionality for the creation and adjacency iteration of a graph data structure.
...
\subsubsection{Methods and operators}
\begin{tabularx}{1\textwidth}{| l | l | X | X |}
  \hline
  \textbf{Expression} & \textbf{Return} & \textbf{Semantics} & \textbf{Conditions} \\
  \endhead
  \hline
  ...
  $x.vertices.begin()$ &  & iterator to the first vertex in global iteration space &  \\ \hline
  $x.vertices.end()$ &  & iterator past the last vertex in global iteration space &  \\ \hline
  $x.vertices.lbegin()$ &  & iterator to the first vertex in local iteration space &  \\ \hline
  $x.vertices.lend()$ &  & iterator past the last vertex in local iteration space &  \\ \hline
  $x.out\_edges.begin()$ &  & iterator to the first outbound edge in global iteration space &  \\ \hline
  $x.out\_edges.end()$ &  & iterator past the last outbound edge in global iteration space &  \\ \hline
  $x.out\_edges.lbegin()$ &  & iterator to the first outbound edge in local iteration space &  \\ \hline
  $x.out\_edges.lend()$ &  & iterator past the last outbound edge in local iteration space &  \\ \hline
  $x.out_edges.vbegin(v)$ &  & iterator to the first outbound edge connected to vertex $v$ in global iteration space &  \\ \hline
  $x.out\_edges.vend(v)$ &  & iterator past the last outbound edge connected to vertex $v$ in global iteration space &  \\ \hline
  $x.out\_edges.vbegin(i_v)$ &  & iterator to the first outbound edge connected to the vertex identified by $i_v$ in global iteration space &  \\ \hline
  $x.out\_edges.vend(i_v)$ &  & iterator past the last outbound edge connected to to the vertex identified by $i_v$ in global iteration space &  \\ \hline
\end{tabularx}
...
\begin{tabularx}{1\textwidth}{| l | l | X | X |}
  ...
  $x.add\_vertex()$ &  & adds a vertex holding a default-constructed instance of \texttt{X::vertex\_attributes\_type} &  \\ \hline
  ...
  $x.add\_edge(v_1, v_2)$ &  & adds an edge between vertices $v_1$ and $v_2$ holding a default-constructed instance of \texttt{X::edge\_attributes\_type} &  \\ \hline
  $x.add\_edge(i_{v1}, i_{v2})$ &  & adds an edge between vertices identified by $i_{v1}$ and $i_{v2}$ holding a default-constructed instance of \texttt{X::edge\_attributes\_type} &  \\ \hline
  $x.remove\_edge(e)$ &  & removes edge $e$ &  \\ \hline
  $x.remove\_edge(e_v)$ &  & removes an edge identified by $e_v$ &  \\ \hline
\end{tabularx}
...
\begin{itemize}
\item $X$, the graph type
\item $VT$, vertex attribute type
\item $ET$, edge attribute type
\item $x$, a value of type X
...
\begin{tabularx}{1\textwidth}{| l | l | X | X |}
  ...
  \endhead  
  \hline
  $x.add\_vertex(a_v)$ &  & adds a vertex holding a copy of $a_v$ &  \\ \hline
  $x.add\_edge(v_1, v_2, a_e)$ &  & adds an edge between vertices $v_1$ and $v_2$ holding a copy of $a_e$ &  \\ \hline
  $x.add\_edge(i_{v1}, i_{v2}, a_e)$ &  & adds an edge between vertices identified by $i_{v1}$ and $i_{v2}$ holding a copy of $a_e$ &  \\ \hline
\end{tabularx}
...
\begin{tabularx}{1\textwidth}{| l | l | X | X |}
  ...
  \hline
  $X(n_v, n_e, it_1, it_2)$ &  & constructs the graph holding $n_v$ vertices and $n_e$ edges given by the iterators $i_1$ and $i_2$ of an edge list container &  \\ \hline
  $x.in\_edges.begin()$ &  & iterator to the first inbound edge in global iteration space &  \\ \hline
  $x.in\_edges.end()$ &  & iterator past the last inbound edge in global iteration space &  \\ \hline
  $x.in\_edges.lbegin()$ &  & iterator to the first inbound edge in local iteration space &  \\ \hline
  $x.in\_edges.lend()$ &  & iterator past the last inbound edge in local iteration space &  \\ \hline
  $x.in_edges.vbegin(v)$ &  & iterator to the first inbound edge connected to vertex $v$ in global iteration space &  \\ \hline
  $x.in\_edges.vend(v)$ &  & iterator past the last inbound edge connected to vertex $v$ in global iteration space &  \\ \hline
  $x.in\_edges.vbegin(i_v)$ &  & iterator to the first inbound edge connected to the vertex identified by $i_v$ in global iteration space &  \\ \hline
  $x.in\_edges.vend(i_v)$ &  & iterator past the last inbound edge connected to to the vertex identified by $i_v$ in global iteration space &  \\ \hline
\end{tabularx}
...
\begin{tabularx}{1\textwidth}{| l | l | X | X |}
  ...
  \hline
  $X(n_v, n_e, it_1, it_2)$ &  & constructs the graph holding $n_v$ vertices and $n_e$ edges given by the iterators $i_1$ and $i_2$ of an edge list container &  \\ \hline
  $x.edges.begin()$ &  & iterator to the first edge (inbound and outbound) in global iteration space &  \\ \hline
  $x.edges.end()$ &  & iterator past the last edge (inbound and outbound) in global iteration space &  \\ \hline
  $x.edges.lbegin()$ &  & iterator to the first edge (inbound and outbound) in local iteration space &  \\ \hline
  $x.edges.lend()$ &  & iterator past the last edge (inbound and outbound) in local iteration space &  \\ \hline
  $x.edges.vbegin(v)$ &  & iterator to the first edge (inbound and outbound) connected to vertex $v$ in global iteration space &  \\ \hline
  $x.edges.vend(v)$ &  & iterator past the last edge (inbound and outbound) connected to vertex $v$ in global iteration space &  \\ \hline
  $x.edges.vbegin(i_v)$ &  & iterator to the first edge (inbound and outbound) connected to the vertex identified by $i_v$ in global iteration space &  \\ \hline
  $x.edges.vend(i_v)$ &  & iterator past the last edge (inbound and outbound) connected to to the vertex identified by $i_v$ in global iteration space &  \\ \hline
\end{tabularx}
