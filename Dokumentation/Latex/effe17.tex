...
This section provides a brief overview of different graph representations in computer sciences discussed in \cite{ita}. Basically, there are two different representations with different but overlapping application areas. 
...
The expressions and semantics of the concepts are based on existing concepts of the \textit{Boost Graph Library} \cite{bgl} as far as applicable. In respect to the related works analysis of \cref{related_work_section}, this can be regarded as best practice in the field of graph processing in C++ applications.
...
\begin{figure}[ht]
  ...
  \includegraphics[width=1\textwidth]{Bilder/plots/local_iter.pdf}
	\caption{Local iteration of vertices (left) and edges (right)}
...
\begin{figure}[ht]
  ...
  \includegraphics[width=1\textwidth]{Bilder/plots/global_iter.pdf}
	\caption{Global iteration of vertices (left) and edges (right)}
...
\begin{figure}[ht]
  ...
  \includegraphics[width=1\textwidth]{Bilder/plots/commit.pdf}
	\caption{Synchronization of memory space after creation of vertices (left) and edges (right)}
	\label{commit_figure}
\end{figure}
...
The results from \cref{cc_experiments_section} and \cref{mst_experiments_section} highlight the problems associated with scaling graph algorithms on distributed systems: Communication hotspots emerge from the fact that many processors require access to the same data which is located in the memory of just a few processors. This data has to be distributed over the interconnect in a one-to-many fashion overloading these processors, while other processors idle. Redistributing this data can help with specific communication hotspots, but as algorithms are composed of several computation and communication steps, such redistributions can lead to new hotspot formations in other parts of the algorithm. For this reason, depending on the algorithm, no optimal scaling solution can be found.

Since the hotspots are mainly communication-centric, they have a higher impact on distributed systems in comparison to shared-memory systems because of the lower throughput and higher latency associated with communication over an interconnect in contrast to data access over a memory bus. For this reason, the discussed algorithms only outperform their shared-memory counterparts with an increase in hardware allocation. 

Still, the algorithms are scalable to a certain extent, allowing to reach faster runtimes with an increase in the amount of participating processors/nodes. Additionally, weak scaling efforts allow the procession of graphs that would not fit in the memory of a single node.

\subsection{Programming Assessment}
\label{programming_assessment_section}
...
\section{Assessment}
The discussed graph concepts allow a wide array of graph algorithms to be implemented on distributed machines. There is however no guarantee that the concepts allow any possible graph algorithm to be implemented because no detailed analysis on all algorithm families could be performed due to the limited scope of this thesis.

Regarding performance, the experimental evaluations of \cref{eval_section} have shown that use cases exist in which algorithms created with the graph concepts can outperform their shared-memory counterparts on distributed systems. This however comes at the expense of a lot of addition hardware that is needed for the computation. For this reason, the decision to choose the discussed concepts' distributed approach over a shared-memory implementation should be carefully planned, when the graph data fits into the memory of an available shared-memory system. 

The PGAS approach of the concepts results in a more streamlined development process in comparison to traditional message passing: Algorithms can be directly ported from their shared-memory counterparts and optimizations can be iteratively applied. Additionally, direct remote access on elements allows for easier debugging. Unfortunately, to reach high performance, optimizations have to be applied that require the developer to contribute additional work which negates the advantage in development time in comparison to implementations with message passing. For this reason, further concepts are required that ease the development process.
...
Finally, the reference implementation of the graph concepts allows for the evaluation of PGAS-related communication models for distributed graph data structures which eases the process of creating graph algorithms specifically oriented towards the PGAS model.
