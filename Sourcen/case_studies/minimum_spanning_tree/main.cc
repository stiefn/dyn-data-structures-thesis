...
int main(int argc, char* argv[]) {
  if(argc == 3) {
    ...
    //dash::LogarithmicVertexMapper<graph_t> mapper(n_vertices, team.size(), 1.5, 0.8);
    dash::BlockedVertexMapper<graph_t> mapper(n_vertices, team.size());
    for(int i = 0; i < 5; ++i) {
      RMATRandomGenerator<graph_t> begin(n_vertices, n_edges, team.size(), 
          team.myid(), mapper, 0.25, 0.25, 0.25, 0.25);
      RMATRandomGenerator<graph_t> end(0, 0, team.size(), team.myid(), mapper, 0, 0, 0, 0);

      std::clock_t g_begin_time = clock();
      graph_t g(begin, end, n_vertices, team, mapper);
      std::clock_t g_end_time = clock();
      if(dash::myid() == 0) {
	std::cout << "[round " << i + 1 << "] construction: " << double(g_end_time - g_begin_time) / CLOCKS_PER_SEC << 
	  std::endl;
      }
      ...
      if(dash::myid() == 0) {
	std::cout << "[round " << i + 1 << "] algorithm: " << double(end_time - begin_time) / CLOCKS_PER_SEC << std::endl;
      }
    }
    dash::finalize();
  }
